...
async function createFeedbackRecord(
  ...
  let feedbackText = payload.message;
  if (payload.service) {
    const unmatched = await unmatchedServiceNames([payload.service]);
    if (unmatched.length > 0) {
      warnings.push(
        `Service "${unmatched.join(", ")}" was not found in the Services table.`,
      );
    }
    feedbackText = `Service: ${payload.service}\n\n${payload.message}`;
  }
  ...
  if (headshotUrl) row.headshot = headshotUrl;
  ...
  return { id: String(data.id), warnings };
}
...
export async function handleFeedbackSubmission(
  ...
  if (payload.headshot) {
    try {
      headshotUrl = await uploadToStorage("feedback/headshot", payload.headshot);
    } catch (err) {
  ...
  const result = await createFeedbackRecord(payload, headshotUrl);
  if ("error" in result) {
